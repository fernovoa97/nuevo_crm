<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col md:flex-row gap-6">
                <div class="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <p class="text-sm text-gray-400 mt-1">Panel de gestión personal</p>
                </div>
            </div>

            <!-- Contadores -->
            <div class="bg-white shadow-sm border border-gray-100 sm:rounded-lg p-6 text-center">
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        <p class="text-sm text-gray-400">Trabajados</p>
                        <p class="text-2xl font-bold text-black">{{ $trabajados }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-400">Pendientes</p>
                        <p class="text-2xl font-bold text-[#00AEEF]">{{ $pendientes }}</p>
                    </div>
                </div>
            </div>

            @if(session('success'))
                <div class="bg-[#e6f7fd] border border-[#00AEEF] text-[#007aaa] px-5 py-4 rounded-2xl shadow-sm">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <p class="text-xs font-semibold text-[#00AEEF] uppercase tracking-wide mb-4">
                    Leads Nuevos ({{ $leadsNuevos->total() }})
                </p>
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm text-black">
                        <thead class="bg-[#00AEEF] text-white">
                            <tr>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($leadsNuevos as $lead)
                                <tr class="hover:bg-[#f0faff] transition" id="lead-row-{{ $lead->id }}">
                                    <td class="p-3">
                                        <button onclick="abrirModal({{ $lead->id }}, '{{ addslashes($lead->nombre) }}', '{{ $lead->dni }}', '{{ $lead->segmento }}', '{{ $lead->telefono1 }}', '{{ $lead->telefono2 }}', '{{ $lead->telefono3 }}', '{{ $lead->telefono4 }}', '{{ $lead->telefono5 }}', '{{ $lead->email }}', '{{ addslashes($lead->comentarios) }}', {{ $lead->movistar ?? 0 }}, {{ $lead->entel ?? 0 }}, {{ $lead->claro ?? 0 }}, {{ $lead->bitel ?? 0 }})"
                                            class="bg-[#00AEEF] hover:bg-[#0099d4] text-white px-2 py-1 rounded-lg text-xs font-semibold transition">✏</button>
                                    </td>
                                                ...
                                                @if($tel)
                                                    <div class="flex items-center gap-1">
                                                        <span class="text-xs font-mono px-2 py-0.5 rounded-lg
                                                            ...
                                                            {{ !$estado ? 'bg-gray-100 text-black' : '' }}">{{ $tel }}</span>
                                                        ...
                                                        @if($estado)
                                                            <button onclick="marcarTelefono({{ $lead->id }}, {{ $i }}, 'null')" class="text-gray-400 hover:text-gray-600 transition text-xs">↺</button>
                                                        @endif
                                                    </div>
                                                @endif
                                            @endfor
                                    ...
                                    <td class="p-3">
                                        <form method="POST"
                                            ...
                                            <select name="tipificacion" required
                                                    class="border border-gray-200 rounded-xl px-3 py-1 text-xs focus:ring-2 focus:ring-[#00AEEF] outline-none">
                                            ...
                                            <button type="submit"
                                                    class="bg-[#00AEEF] hover:bg-[#0099d4] text-white px-3 py-1 rounded-xl text-xs transition">
                                                Guardar
                                            </button>
                                        </form>
                                    </td>
            ...
            <!-- ================= SEGUIMIENTO ================= -->
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 border-l-4 border-l-[#00AEEF]">
                <p class="text-xs font-semibold text-[#00AEEF] uppercase tracking-wide mb-4">
                                ...
                                <tr class="hover:bg-[#f0faff] transition">
                                    ...
                                    <td class="p-3">
                                        <div class="flex flex-col gap-1">
                                            @for($i = 1; $i <= 5; $i++)
                                                @if($lead->{"telefono$i"})
                                                    <span class="text-xs font-mono px-2 py-0.5 rounded-lg bg-gray-100 text-black">{{ $lead->{"telefono$i"} }}</span>
                                                @endif
                                            @endfor
                                        </div>
                                    </td>
                                    <td class="p-3 font-semibold text-[#00AEEF]">
                    ...
                    <table class="min-w-full text-sm text-black">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($leadsTrabajados as $lead)
                                <tr class="hover:bg-[#f0faff] transition {{ $lead->tipificacion === 'No interesado' ? 'bg-red-50' : '' }}">
                                    <td class="p-3">
                                        <button onclick="abrirModal({{ $lead->id }}, '{{ addslashes($lead->nombre) }}', '{{ $lead->dni }}', '{{ $lead->segmento }}', '{{ $lead->telefono1 }}', '{{ $lead->telefono2 }}', '{{ $lead->telefono3 }}', '{{ $lead->telefono4 }}', '{{ $lead->telefono5 }}', '{{ $lead->email }}', '{{ addslashes($lead->comentarios) }}', {{ $lead->movistar ?? 0 }}, {{ $lead->entel ?? 0 }}, {{ $lead->claro ?? 0 }}, {{ $lead->bitel ?? 0 }})"
                                            class="bg-[#00AEEF] hover:bg-[#0099d4] text-white px-2 py-1 rounded-lg text-xs font-semibold transition">✏</button>
                                    </td>
                                    ...
                                    <td class="p-3">
                                        @if($lead->tipificacion === 'No interesado')
                                            <form method="POST"
                                                ...
                                                <button type="submit"
                                                        class="bg-black hover:bg-gray-800 text-white px-3 py-1 rounded-xl text-xs transition">
                                                    Cambiar
                                                </button>
                                            </form>
                                        @else
            ...
            <!-- ================= PROPUESTAS / VENTAS ================= -->
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 border-l-4 border-l-black">
    ...
    <div id="modal-editar" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
        <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-lg mx-4">
            <h3 class="text-lg font-bold text-black mb-6">Editar Lead</h3>
            <form id="form-editar" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="nombre" id="edit-nombre" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-500 uppercase">DNI</label>
                        <input type="text" name="dni" id="edit-dni" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-500 uppercase">Segmento</label>
                        <input type="text" name="segmento" id="edit-segmento" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-500 uppercase">Email</label>
                        <input type="email" name="email" id="edit-email" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    @for($i = 1; $i <= 5; $i++)
                        <div>
                            <label class="text-xs font-semibold text-gray-500 uppercase">Teléfono {{ $i }}</label>
                            <input type="text" name="telefono{{ $i }}" id="edit-telefono{{ $i }}" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                        </div>
                    @endfor
                </div>
                <div>
                    <label class="text-xs font-semibold text-gray-500 uppercase">Comentarios</label>
                    <textarea name="comentarios" id="edit-comentarios" rows="3" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none resize-none"></textarea>
                </div>
                <div class="grid grid-cols-4 gap-4">
                    <div>
                        <label class="text-xs font-semibold text-gray-500 uppercase">Movistar</label>
                        <input type="number" name="movistar" id="edit-movistar" min="0" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-500 uppercase">Entel</label>
                        <input type="number" name="entel" id="edit-entel" min="0" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-500 uppercase">Claro</label>
                        <input type="number" name="claro" id="edit-claro" min="0" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-500 uppercase">Bitel</label>
                        <input type="number" name="bitel" id="edit-bitel" min="0" class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                    </div>
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="cerrarModal()" class="bg-gray-100 hover:bg-gray-200 text-black px-5 py-2 rounded-xl text-sm font-semibold transition">Cancelar</button>
                    <button type="submit" class="bg-black hover:bg-gray-900 text-white px-5 py-2 rounded-xl text-sm font-semibold transition">Guardar cambios</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-seguimiento" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
        <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm mx-4">
            <h3 class="text-lg font-bold text-black mb-6">📅 Agendar Seguimiento</h3>
            <form id="form-seguimiento" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="text-xs font-semibold text-gray-500 uppercase">Fecha y hora</label>
                    <input type="datetime-local" name="fecha_seguimiento" id="input-fecha-seguimiento" required
                           class="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-[#00AEEF] outline-none">
                </div>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="cerrarModalSeguimiento()" class="bg-gray-100 hover:bg-gray-200 text-black px-5 py-2 rounded-xl text-sm font-semibold transition">Cancelar</button>
                    <button type="submit" class="bg-[#00AEEF] hover:bg-[#0099d4] text-white px-5 py-2 rounded-xl text-sm font-semibold transition">Agendar</button>
                </div>
            </form>
        </div>
    </div>
